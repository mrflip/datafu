...
queue()
    ...
    .defer(d3.tsv,  "ballparks-some.tsv")
    .await(ready);
...
function ready(error, us, ballparks) {
  ...
  voronoi_geo(ballparks)
      .forEach(function(cell) {
        ...
        geojson_parks.features.push({ type: "Point", coordinates: [pt.lng, pt.lat], id: pt.park_id });
        // geojson_parks.features.push({ type: "Polygon", coordinates: [cell_ring], id: pt.park_id+"_cell" });
        
      });

  var dumpy = d3.select("#voronoi-dump")
    .text(JSON.stringify(geojson_cells));
  var dumpy = d3.select("#places-dump")
    .text(JSON.stringify(geojson_parks));
